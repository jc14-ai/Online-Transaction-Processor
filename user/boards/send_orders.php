...
if (isset($_GET['user_id'])) {
    ...
    $user_cart_query = "SELECT cart.*, orders.order_id FROM cart LEFT JOIN orders ON cart.user_id = orders.user_id WHERE cart.user_id = $user_id;";
    $user_cart_query_run = mysqli_query($conn, $user_cart_query);

    while ($user_cart_row = mysqli_fetch_assoc($user_cart_query_run)) {
        $order_id = empty($user_cart_row['order_id']) ? NULL : $user_cart_row['order_id'];
        $coffee_id = empty($user_cart_row['coffee_id']) ? NULL : $user_cart_row['coffee_id'];
        $coffee_size_id = empty($user_cart_row['coffee_size_id']) ? NULL : $user_cart_row['coffee_size_id'];
        $donut_id = empty($user_cart_row['donut_id']) ? NULL : $user_cart_row['donut_id'];
        $bundle_id = empty($user_cart_row['bundle_id']) ? NULL : $user_cart_row['bundle_id'];
        $quantity = empty($user_cart_row['quantity']) ? NULL : $user_cart_row['quantity'];
